...
export const createJournalEntry = async (entry: JournalEntryInput) => {
  try {
    ...
    try {
      ...
      console.log(`Entry categorized as: ${categorization.categoryTitle} (${categorization.action})`);
...
export const fetchJournalEntries = async (userId: string): Promise<USER_JOURNAL_ENTRIES_QUERYResult> => {
  try {
    ...
    if (updates.content || updates.title) {
      try {
        ...
        if (currentEntry && currentEntry.userId) {
          ...
          categoryId = categorization.categoryId;
          console.log(`Entry re-categorized as: ${categorization.categoryTitle} (${categorization.action})`);
          console.log(`Reasoning: ${categorization.reasoning}`);
        }
      } catch (categorizationError) {
        console.error("Re-categorization failed, continuing without updating category:", categorizationError);
...
export const getJournalEntryById = async (entryId: string): Promise<JOURNAL_ENTRY_BY_ID_QUERYResult> => {
  try {
...
export const fetchJournalEntriesWithDateRange = async (
  ...
  try {
    const entries = await sanityClient.fetch(USER_JOURNAL_ENTRIES_WITH_DATE_RANGE_QUERY, {
      userId,
      startDate,
      endDate,
    });
