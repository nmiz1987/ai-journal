 ...
 */
const toDateString = (date: Date | string): string => new Date(date).toISOString().split("T")[0];

/**
 * Gets unique entry dates sorted newest first
 */
const getUniqueDates = (entries: JournalEntry[]): string[] =>
  [...new Set(entries.map((entry) => toDateString(entry.createdAt)))].sort((a, b) => b.localeCompare(a));
...
const daysBetween = (date1: string, date2: string): number =>
  Math.floor((new Date(date1).getTime() - new Date(date2).getTime()) / (1000 * 60 * 60 * 24));
...
const calculateCurrentStreak = (entryDates: string[]): { streak: number; dates: string[] } => {
  ...
  const generateConsecutiveDates = (start: string): string[] => Array.from({ length: entryDates.length }, (_, i) => addDays(start, -i));
...
export const getStreakStatusMessage = ({ currentStreak, lastEntryDate }: StreakData): string => {
  ...
  const messages = {
    1: hasEntryToday ? "Great start! Keep it going tomorrow! 🔥" : "1 day streak - write today to continue! 💪",
    default: hasEntryToday ? `Amazing! ${currentStreak} day streak! 🔥` : `${currentStreak} day streak - write today to continue! 🔥`,
  };
...
export const getDaysUntilNextMilestone = (
  ...
  const nextMilestone = milestones.find((milestone) => currentStreak < milestone) ?? Math.ceil(currentStreak / 100) * 100;
